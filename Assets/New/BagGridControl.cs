...
public class BagGridControl : MonoBehaviour
{
    ...
    public static void updateItemToUI()
    {
        for(int j=0;j<bagGridControl.myBag.Length;j++)
        {
            ...
            foreach(var item in bagGridControl.bagItem.itemsList_a)
            {
                insertItemToUI(item);
            }
            foreach(var item in bagGridControl.bagItem.itemsList_b)
            {
                insertItemToUI(item);
            }
            foreach(var item in bagGridControl.bagItem.itemsList_c)
            {
                insertItemToUI(item);
            }
